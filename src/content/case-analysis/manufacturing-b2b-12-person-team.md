---
title: "Manufacturing B2B: ทีม 12 คน เพิ่ม Pipeline Velocity +30% ลด Discount Rate 50% ใน 6 เดือน"
description: "Case จริง: Manufacturing B2B ทีม 12 คน — pipeline velocity +30%, discount rate 15%→7%, inquiry response 2วัน→15นาที, margin +15%, saving ฿3.8M/ปี"
lang: th
...
- Pipeline velocity: ดีล stage เฉลี่ย stay 45 วัน (มนุษย์ follow-up ไม่ทัน)
- เจ้าของทำงาน 14 ชม./วัน ไม่มี weekend

**จุดชนวน:** เจ้าของขอคำปรึกษาเพราะรู้สึก "ขาย HubSpot ออกไปแล้ว แต่ปัญหายังอยู่เหมือนเดิม"

## สิ่งที่ผมทำให้ (6 เดือน Monthly Consulting)

### เดือน 1: Audit + Stack Migration

- Audit ทุก workflow ทีมขายทำเอง — ลิสต์ 18 งานซ้ำๆ
- Export ข้อมูลลูกค้าจาก CRM SaaS เดิมเป็น CSV → migrate ไป Google Sheets + Apps Script
...
**Workflow 2: Proposal Auto-Generator**
...
- Focus: discovery + objection handling ไม่พึ่งส่วนลด
- Role-play 3 รอบด้วยเคสจริง
- Post-training KPI tracking 30 วัน

### เดือน 5-6: KPI Dashboard + Scale

- Real-time KPI dashboard (Google Sheets + Apps Script)
- Commission structure ใหม่ 3-tier (base + variable tier + team bonus)
- Train owner ใช้ dashboard เอง
- Handover documentation 40+ หน้า
...
| Pipeline velocity (stage duration) | 45 วัน | 31 วัน | -31% |
| Discount rate | 15% | 7% | -53% |
| Closing rate | 22% | 31% | +41% |
| Avg deal size | ฿240k | ฿310k | +29% |
| Revenue (YoY same quarter) | baseline | +28% | — |
...
- Margin per deal: +15% (discount rate ลดเป็นหลัก)
...
### Client Testimonial (quoted, anonymized)
...
## Stack ที่ใช้ (client maintain ต่อเองได้)
...
- **Google Drive** — Document storage (proposals, SOPs)

**ไม่ใช้:** Enterprise CRM, BI license, proprietary AI platform, monthly retainer

## Total Investment vs ROI

- **Investment:**
  - AI Workshop ฿50,000 (Workshop + Follow-up 30 วัน)
  - Monthly Consulting 6 เดือน × ฿30,000 = ฿180,000
  - Sale Training Bundle ฿52,800
  - Total: **฿282,800**
...
## Lessons learned ที่ผมเอามา apply กับ client อื่น
...
**2. Migrate ออกจาก SaaS ล็อคก่อน**
ถ้าทีมยังติด SaaS ล็อค การ build workflow ใหม่จะซ้อนกันสับสน ต้อง cancel SaaS เดิมก่อน migrate data
...
- [Pillar: Is your org using AI for real](/insights/is-your-org-using-ai-for-real) — 10 คำถามที่ client นี้เคยตอบ "ใช่" 8 ข้อ (ตอนเริ่ม) และเหลือ 1 ข้อ (ตอนจบ)
- [Commission structure ทีม 10 คน](/faq) — formula ที่ใช้กับ client นี้
- [Workshop vs Consulting ต่างกันยังไง](/faq) — client นี้เลือก monthly consulting หลังลอง workshop
...
- [AI Workshop + Follow-up ฿50,000](/services/ai-workshop-followup) — Starting point ของ client นี้
- [Sale Training Bundle ฿52,800](/services/sale-training-bundle) — Module ที่ 4 ของเคสนี้
- [กรอกฟอร์ม 3 นาที](/intake-form) — ผมอ่านแล้วบอกว่าเคสคุณ match กับ client นี้แค่ไหน + ควร start ตรงไหน
