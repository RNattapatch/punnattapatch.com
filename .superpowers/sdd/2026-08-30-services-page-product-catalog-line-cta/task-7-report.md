...
6. The original browser file uses `node:test`, so `pnpm exec playwright test tests/services-vnext.spec.ts` cannot discover Playwright-runner tests. The exact command was executed and preserved as evidence; per the Phase 7 brief, `node --test tests/services-vnext.spec.ts` is authoritative and passes 6/6.
...
| Browser behavior | Pass — 6/6 via Node-owned Playwright harness | `evidence/task-7/services-browser.log` |
| Playwright CLI compatibility | Adapted — exact CLI reports `No tests found`; Node harness is authoritative | `evidence/task-7/playwright-runner.log` |
...
| Accessibility | Pass — Nav → Hero → Cards → Chooser → FAQ → Final CTA → Floating LINE; reduced motion has no perpetual animation | `evidence/task-7/services-browser.log` |
...
| Analytics | Pass — one event/provider with complete props; thrown Plausible/Meta errors do not block TikTok or navigation | `evidence/task-7/services-browser.log` |
| Legacy anchors | Pass — `#sales-team-structure`, `#ai-agent-ceo`, `#offer-c1`, and all six chooser targets resolve uniquely | `evidence/task-7/services-browser.log` |
...
pnpm exec playwright test tests/services-vnext.spec.ts  # incompatibility captured
node --test tests/services-vnext.spec.ts                # authoritative: 6/6
```
