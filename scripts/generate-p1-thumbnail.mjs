...
const htmlPath = resolve(scriptDir, 'p1-service-thumbnail.html');
const sourcePhoto = resolve(root, 'public/lp/inhouse/hands-on.jpg');
const pngPath = resolve(root, 'src/assets/services/product-thumbnails/p1-ai-sales-agent-bootcamp.png');
const jpgPath = resolve(root, 'public/services/thumbs/p1-ai-sales-agent-bootcamp.jpg');

const html = await readFile(htmlPath, 'utf8');
if (!html.includes('../public/lp/inhouse/hands-on.jpg')) throw new Error('P1 thumbnail must use the approved real class photograph');
...
try {
  ...
  const audit = await page.locator('#p1-thumbnail').evaluate((artboard) => {
  ...
  await page.locator('#p1-thumbnail').screenshot({ path: pngPath, type: 'png', animations: 'disabled' });
